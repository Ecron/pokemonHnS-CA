...
- [Pokémon ハート&ソウル](https://github.com/shadowmountain888/pokemonHnS/tree/jp-localization) (Pokémon Heart&Soul, Japanese version)

### Translation Tips and Tricks
- For regular in-game dialogues, max. string length would be around 40 latin characters. Any sentence longer than that would need to be broken to the next line.
- Regarding the name of each Gamemode setting, at the beginning of a new game, the max. string length for the settings' names is recommended to be of 15 or 16 latin characters. This also applies to the settings' names in the Options menu.
- Regarding the bottom explanations of each Gamemode setting, a good rule would be not to exceed 35 or 36 latin characters, depending on the number of capital letters (maximum 38 chars). A {PKMN} code can be used to insert a 2-character PKMN symbol.
...
- Maybe your language contains any latin characters that are not present in the English charset. If you try to use that specific Unicode character in any translation, the game won't build, it will return an error saying that character is unknown. This can be easily fixed by adding the "special" characters to the `charmap.txt` file.
  - **CAUTION**: This file works like kind of a map, where every character is given an HEX code that represent a place inside the `../graphics/fonts/latin_*.png` file (or equivalent for each language). So, for example, for Catalan translation, we need to add both the Ŀ and ŀ characters (for words like the verbs "Canceŀlar", "Soŀlicitar" or the proper name "Gaŀla"). The correct way to represent this special character is using 3 different characters: L + middle dot (·) + L. But as the Ŀ character does exist, we can use it so we gain 1 precious character, very useful given POKéMON's tight space restrictions. So, we assigned the HEX value `2F` to the Ŀ character in the `charmap.txt` file, so the game will look for the graphical representation of that character right next to the + symbol on the `../graphics/fonts/latin_normal.png`  file. We then just need to edit that PNG file and draw the Ŀ character, and the game will represent it, but more important, recognize it.
  - **NOTE**: If you modify a character in the graphics file, its width won't be modified, and we still didn't find out how to do so.
